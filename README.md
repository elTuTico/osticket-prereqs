...
<p>
Now comes the download, installation, and file configuration of osTicket itself. Once the zip file of osTicket have been downloaded, extract and copy the “upload” folder into (C:) Drives\inetpub\wwwroot folder. This is a folder that acts like the web server’s main folder, which is why it’s important that the server knows where to go when running osTicket. Which is why it must be renamed from the “upload” folder within wwwroot to “osTicket”.
</p>
<br />
...
<p>
To enable those extensions head back to your IIS window and drop down on Site, Default, and double click on osTicket. From there double click on PHP Manger and open up “Enable or disable an extension”. Enable php_imap.dll, php_intl.dll, and php_opcache.dll. Finish by refreshing the osTicket site in the browser and observe the changes. 
</p>
<br />
...
<p>
Now before continuing to setup osTicket in the browser first finish configuring the files for the program to run smoothly on the server. Open up a File Explorer window for the (C:) Drive and navigate to inetpub, wwwroot,osTicket,include, and search for “ost-sampleconfig.php” which will be renamed to “ost-config.php”. Right click on that newly named file, click on Properties, navigate to security. From here Disable inheritance/Remove All followed by making a New Permission for Everyone/All.
</p>
<br />
...
<p>
Continue setting up osTicket on the web browser by clicking Continue. Configure the name, default emails, and such, but ensure that the information inputted for MySQL matches the database about to made once HeidiSQL has been downloaded. Install the application by following the prompts, open the program, and Create a new session with the credentials “User: root” and “Password: Password1”. Click connect to the session, right click under “Unnamed”, and create a new database called “osTicket”.
</p>
<br />
...
<p>
Once the Settings for osTicket have been filled out in the browser with the MySQL Database: osTicket, MySQL Username: root, and MySQL Password: Password1 click “Install Now!”. If there were no errors there should be a fully functioning ticketing system setup on your computer’s localhost web server. Which would give a user access to a help desk login page: http://localhost/osTicket/scp/login.php and an End User page: http://localhost/osTicket/. 
</p>
<br />
...
<p>
Lastly clean up of certain files must be done before configuring. Navigate to Roles, Departments, and Teams within osTicket. Navigate to (C:) Drive\inetpub\wwwroot\osTicket\setup and delete this file. Next change the Permissions to “Read” only for (C:) Drive\inetpub\wwwroot\osTicket\include\ost-config.php. 
</p>
<br />
